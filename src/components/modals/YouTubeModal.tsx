...
import { X, Youtube, AlertCircle, ExternalLink, Search } from 'lucide-react';
import { fetchYouTubeResources, Resource } from '../../services/resourceService';
...
export function YouTubeModal({ language, isOpen, onClose }: YouTubeModalProps) {
  ...
  async function loadResources() {
    try {
      setLoading(true);
      setError(null);
      const data = await fetchYouTubeResources(language);
      setResources(data);
    } catch (err) {
      console.error('Error loading YouTube resources:', err);
              ...
              <div className="text-center py-10">
                {searchQuery ? (
                  <div className="text-gray-400">
                    <p className="mb-2">No resources matching "{searchQuery}"</p>
                    <button 
                  ...
                  <div className="text-gray-400">
                    <p>No YouTube resources available for {language} yet.</p>
                ...
                {filteredResources.map((resource) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-medium text-white text-lg truncate group-hover:text-cyan-300 transition-colors">
                        {resource.title}
                      </h3>
                      <p className="text-gray-400 text-sm truncate">{resource.url}</p>
                    </div>
                    <ExternalLink className="h-5 w-5 text-gray-400 group-hover:text-cyan-300 transition-colors" />
                  </a>
                ))}
              </div>
            )}
